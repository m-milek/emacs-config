...
#+END_SRC
* Environment
On macOS, the environment Emacs runs in, doesn't contain all the evn variables I'd expect it to. This loads them.
#+BEGIN_SRC elisp
...
#+END_SRC
* My Elisp
These are Emacs Lisp files that I've written for myself.
#+BEGIN_SRC elisp
...
* Font
Setup different font faces for text and code
#+BEGIN_SRC elisp
  (let ((font-size 150))
...
* Tree-sitter
I'm not using the builtin treesit implementation from Emacs 29+, since I find those grammars subpar compared to the tree-sitter third party package.
For pretty much every single language, the third part library has better syntax highlighting.
#+BEGIN_SRC elisp
...
** C/C++
Sometimes I'll have to tell flycheck where some external libraries are located
#+BEGIN_SRC elisp
...
#+END_SRC
** Common Lisp
Long startup time, not used for now.
#+BEGIN_SRC elisp
...
* Keyfreq
Record which commands are used most frequently
#+BEGIN_SRC elisp
  (use-package keyfreq
    :ensure t
    :config
